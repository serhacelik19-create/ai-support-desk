...
export default function AnalyticsLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/dashboard", label: "Konsol", icon: LayoutDashboard },
  ];
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", background: "var(--bg-primary)" }}>
        <div style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Yükleniyor...</div>
      </div>
    );
  }
          ...
          <div className="top-nav-right">
            ...
            <div className="nav-user-info" style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", marginRight: "4px" }}>
              ...
              <span style={{ fontSize: "10px", color: "var(--text-tertiary)", textTransform: "capitalize" }}>
                {user?.role === "admin" ? "Yönetici" : "Destek Temsilcisi"}
              </span>
            </div>
            <div className="nav-user-avatar notranslate" title={user?.name}>
              {getUserInitials()}
            </div>
            <button onClick={handleLogout} className="top-nav-link nav-logout" style={{ background: "transparent", border: "none", cursor: "pointer" }} title="Çıkış Yap">
